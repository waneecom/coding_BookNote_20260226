...
import { supabase } from './supabase'; // ✅ [추가] Supabase 클라이언트

const SAVE_ID = 'main_save'; // Supabase 테이블의 고정 row id
...
export default function AppWithCloud() {
  // --- 상태 관리 ---

  const [isAppLoading, setIsAppLoading] = useState(true); // ✅ [추가] 클라우드 로딩 상태

  // 초기값을 null로 설정 (클라우드에서 불러오기 전까지 대기)
  ...
  const isInitialized = useRef(false); // 초기 로드와 사용자 변경을 구분

  // ✅ [추가] 앱 시작 시 Supabase에서 데이터 불러오기
  useEffect(() => {
  ...
  // 초기 로드 완료 후 isInitialized 활성화
  // (auto-save effect보다 나중에 정의 → 같은 렌더에서 auto-save가 먼저 실행되어 초기 로드를 건너뜀)
  useEffect(() => {
    if (!isAppLoading) {
  ...
  // 데이터 변경 시 2초 디바운스 후 자동 저장
  useEffect(() => {
    if (!isInitialized.current || !databases) return;
  ...
  const handleInitialSetup = async () => {
    ...
    // 클라우드 저장
    await supabase.from('booknote_saves').upsert({ id: SAVE_ID, data: newDb });
  };
    ...
    <aside className={`w-64 border-r ${currentTheme.border} ${currentTheme.panel} flex flex-col z-20 shadow-sm shrink-0 h-full`}>
      <div className={`p-5 border-b ${currentTheme.border} flex flex-col gap-4`}>
        <div className="flex justify-between items-center">
          ...
          {/* 자동저장 상태 표시 */}
          <div className={`flex items-center gap-1 text-[11px] font-bold px-3 py-1.5 rounded-lg transition-all duration-300 ${isSaved === true ? 'bg-green-100 text-green-700' : isSaved === 'saving' ? 'bg-blue-50 text-blue-500' : 'bg-black/5 opacity-40'}`}>
            {isSaved === true
              ? <><Check size={13}/> 저장됨</>
              : isSaved === 'saving'
              ? <><Save size={13} className="animate-pulse"/> 저장 중...</>
              : <><Save size={13}/> 자동저장</>
            }
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {books.map(book => (
                    <motion.div key={book.id} onClick={() => { setSelectedBook(book); setViewMode('chapters'); }} onContextMenu={e => handleContextMenu(e, book)} className={`relative p-6 rounded-3xl border ${currentTheme.border} ${currentTheme.panel} shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group flex flex-col h-64`}>
                      <div className="flex justify-between items-start mb-4"><span className={`text-[10px] font-bold px-2 py-1 rounded-full ${book.status==='읽는 중'?'bg-blue-100 text-blue-600':'bg-black/5'}`}>{book.status}</span></div>
